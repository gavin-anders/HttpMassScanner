...
def extract_path(url):
    """
    extracts path part from url string
    """
    path = None
    p = re.compile(".*:\/\/.*(\/.*)$")
    result = p.search(url)
...
def parse_url_string(url):
    ...
    if path is None:
        path = '/'
    hostname = u.netloc
    return hostname, port, path, is_ssl

def format_template(template, placeholders):
    # https://pyformat.info/#named_placeholders
    byteplaceholders = {}
    for oldkey, oldval in placeholders.items():
        byteplaceholders[oldkey.encode()] = oldval.encode()
    request = b''
    request = template % byteplaceholders
    return request

def post_results(callback, rawrequest, status, rawresponse=None, headers=None, content=None):
    pass

# Function to be executed in a thread
def send_request(url, settings):
    # process the url
    host, port, path, isssl = parse_url_string(url)

    # process the settings
    template = base64.b64decode(settings["template"])
    placeholders = settings["placeholders"]
    config = settings["config"]
    callback = settings["callback"]

    # format the template
    rawrequest = format_template(template, placeholders)

    req = RawRequest(host, port, rawrequest, is_ssl=isssl, verbose=config["verbose"], follow_redirect=config["follow_redirects"])
    if "timeout" in config.keys():
        req.set_timeout(config["timeout"])
    ...
    if response:
        ...
        status = None
        headers = []
        post_results(callback, rawrequest, response.status)
    return

if __name__ == "__main__":
    import sys
    """
    This tool is designed to accept a list of targets/urls, a template request, 
    and a json array of settings. Threads are spun off that make a single request
    and use the template request and settings file to craft a valid http request
    """
    print("[+] Starting Mass HTTP Scanner")
    ...
    try:
        ...
        #pool = ThreadPool(20)
        #for url in targets:
        #    pool.add_task(send_request, url, settings)
